...
type BengkelRepositoryInterface interface {
	...
	FindBengkelById(bengkelId string, page, size int) (*models.Bengkel, []models.BengkelTestimonial, int, error)
...
func (*BengkelRepository) FindBengkelById(bengkelId string, page, size int) (*models.Bengkel, []models.BengkelTestimonial, int, error) {
	...
	err = db.GetDB().Model(&models.BengkelTestimonial{}).Where("bengkel_id = ?", bengkelId).Count(&count).Error
	if err != nil {
		return nil, nil, 0, err
	}
...
func (*BengkelRepository) GetBengkelById(bengkelId string) (*models.Bengkel, error) {
	...
	// Try cache first
	var bengkel models.Bengkel
	if err := cache.Get(cacheKey, &bengkel); err == nil {
		return &bengkel, nil
	}
	...
	// Cache for 5 minutes
	cache.Set(cacheKey, bengkel, 5*time.Minute)
	return &bengkel, nil
}
